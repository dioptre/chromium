...
bool DevToolsSSLServerSocket::InitializeSSLContext() {
  ...
  base::Time not_before = base::Time::Now() - base::Minutes(1);
  ...
  ssl_context_ = net::CreateSSLServerContext(std::move(credentials), ssl_config);
  return ssl_context_.get() != nullptr;
}
